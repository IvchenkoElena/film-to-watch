...
public class FilmDbStorage extends BaseRepository<Film> implements FilmStorage {
    ...
    public List<Film> bestFilms(int count, Integer genreId, Integer year) {
        String query = """
                    ...
                    FROM FILMS f JOIN RATING r ON f.RATING_ID = r.RATING_ID LEFT
                    JOIN FILM_GENRE fg ON f.FILM_ID = fg.FILM_ID
                    WHERE (FG.GENRE_ID = ? OR ? IS NULL)
                    AND (EXTRACT(YEAR FROM release_date) = ? OR ? IS NULL)
                    ORDER BY f.likes_count DESC LIMIT ?
                    """;
        return findMany(query, genreId, genreId, year, year, count);
    }
